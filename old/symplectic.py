...
def find_symplectic(Lambda1, Lambda2):
    ...
    f = f[: len(f) // 2]
    f.sort()
    # ap, d, bp = blochmessiah(R)
    # squeezers = np.log(d.diagonal())[:n]
    ...
    sigma1 = np.zeros((n, n), dtype=Lambda1.dtype)
    sigma2 = np.zeros((n, n, n, n), dtype=Lambda2.dtype)
    for i in range(n):
        for j in range(n):
            sigma1[i, j] += (1 / 2) * (
                Lambda1[i, j]
    ...
    V, g = allfock.find_V_fock(sigma1, sigma2)
    # V, g = allfock.find_V_fock(sigma1 + np.diag(f) - 1, sigma2)

    # this may fail when there is noise
...
if __name__ == "__main__":
    ...
    for n in range(1, 5):
        print(f"Testing n = {n}")
        t0 = time.time()
        print("Errors:", [round(x, 10) for x in errors_random(n)])
